...
for Test_turn in xrange(9):
    skip_window = 32*(2**(Test_turn))
    ...
    plt.figure(1)
    plt.cla()
    line_label = str(skip_window)+",Traing Turn "+str(Test_turn)
    ...
    plt.text(10,120,topic_train ,fontproperties=font)
    plt.text(4000,60, r'skip_window: '+str(num_skips))
    plt.text(4000,50, r'runtime(sec):'+runtime.decode('utf-8'))
    plt.text(4000,40, r'final loss='+str(y[len(y)-1]))


    plt.savefig("./images/"+line_label+".png")
    plt.figure(2)
    plt.plot(x,y,label=line_label)
plt.savefig("./images/main_"+line_label+".png")
